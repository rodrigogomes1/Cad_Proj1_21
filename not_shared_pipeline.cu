...
int main(int argc, char *argv[]) {
    ...
    {
        int offset = (imgh/NUM_STREAMS)*i;
        cudaMemcpyAsync(&d_in[offset], &img[offset], sizeof(int)*3*imgw*((imgh/NUM_STREAMS)), cudaMemcpyHostToDevice, streams[i]);

        dim3 nb(imgw+(BLOCK_W-1)/BLOCK_W,(imgw*((imgh/NUM_STREAMS)))+(BLOCK_H-1)/BLOCK_H);
        dim3 th(BLOCK_W,BLOCK_H);

        averageImg<<<nb, th,0,streams[i]>>>(d_out,d_in,offset,imgw, imgh,d_filter,alpha);

        YourKernel<<<grid,block,smemSize,streams[curStream]>>>(...);
        cudaMemcpyAsync(&out[offset], &d_out[offset], sizeof(int)*3*imgw*((imgh/NUM_STREAMS)), cudaMemcpyDeviceToHost, streams[i]);
    }
